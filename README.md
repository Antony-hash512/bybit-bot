...
> 💡 **Автовыбор ключей и настроек**: В зависимости от `USE_TESTNET` (True/False) и значений `DRY_RUN`, `SPREAD_PERCENT`, `SAVINGS_PERCENT`, бот автоматически загружает соответствующую конфигурацию и торговую математику из `.env`.
...
- **Встречный ордер**: Рассчитывает средневзвешенную цену продажи, делает скидку `SPREAD_PERCENT` (по умолчанию **1.25%**: `buy_price_wbtc = avg_sell_price * (1 - SPREAD_PERCENT / 100)`), удерживает `SAVINGS_PERCENT` в USDT для комиссий и долларовой копилки (по умолчанию **0.25%**: `safe_usdt = total_usdt * (1 - SAVINGS_PERCENT / 100)`), жестко округляет значения ВНИЗ через `math.floor` (цена до 1 знака: `math.floor(p * 10) / 10`, объем до 6 знаков: `math.floor(q * 1000000) / 1000000`) и выставляет лимитный ордер **BUY** на `WBTCUSDT`.
